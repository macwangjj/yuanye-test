...
新版测试站会先检查原始导出图，再按结果进入 AI Offset 修缝、`repairSeams` 或重生。AI 修缝会保留更好的版本，必要时做第二次细修；本地融合会保留内侧纹理，减少硬边、糊边和平涂条带。当前质量闸会额外模拟 2×2 平铺预览，直接检测平铺中心线、四角交汇点、光晕和假边带，并扫描上下/左右边缘是否存在整体像素漂移，同时比较接缝带与内区的高频细节，避免“单张看不出、平铺后明显”以及“接上了但边缘被抹糊”的误判；导出端也会做基础印花清晰度、低清放大痕迹、色阶断层、压缩块噪点、锐化光晕、可打印细节密度、花型信息量、布局均衡、镜像轴痕、预平铺输出、画框留白和导出比例拉伸检查。若上游返回方形无缝源图，导出端会先尝试用低失真的 2×3 周期网格转为竖版，再继续复核接缝；如果内部网格线明显，仍会失败。系统会复核实际 JPG 是否为 `4961 × 7559 px`、300dpi JFIF 元数据，拒绝明显模糊、像素复制式放大、阴影硬色带、方块压缩脏噪、过锐化白边/黑边、细节密度过低、几乎空白、单个主体居中过重、机械镜面对称轴、误输出 2×2 预览图、白边/黑边/画框、强行拉伸变形或规格不正确的成品。未通过当前完整质检认证的图片只能预览、复检或继续修复，不会开放单张下载和批量打包；当前任务和历史记录都必须带有完整认证元数据，并且明确保存“竖版直出”或“周期转竖版 2×3”的导出路径，旧版不完整记录会显示为未认证。更完整的修复策略见 [docs/SEAM_REPAIR.md](docs/SEAM_REPAIR.md)。
