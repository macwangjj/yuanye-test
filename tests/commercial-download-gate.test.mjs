...
test("history and batch downloads only include certified records", () => {
  ...
  assert.match(recordCertificationFunction, /typeof gate\.compressionArtifactScore === "number"/, "history certification must require the compression artifact gate");
  assert.match(recordCertificationFunction, /typeof gate\.sharpenHaloScore === "number"/, "history certification must require the sharpen halo gate");
  assert.match(recordCertificationFunction, /typeof gate\.fullSizeEdgeScore === "number"/, "history certification must require the final-size edge gate");
...
test("history certification rejects stale or partial metadata", () => {
  ...
  const certifiedRecord = {
    ...
    certification: {
      ...
      gate: {
        ...
        compressionArtifactScore: 1.8,
        sharpenHaloScore: 0.6,
        fullSizeEdgeScore: 1.1,
        richnessScore: 9.8,
  ...
  }), false, "records missing the seam detail-loss, upscale-artifact, posterization, compression-artifact, sharpen-halo, final-size edge, richness, layout, mirror-axis, pre-tiled, texture-density, outer-frame, and aspect-warp gates should not be downloadable");
  assert.equal(recordHasCertifiedDownload({
    ...certifiedRecord,
    certification: {
      ...certifiedRecord.certification,
...
test("saved history records retain print certification metadata", () => {
  ...
  assert.match(appSource, /compressionArtifactScore: typeof check\.compressionArtifact\?\.blockScore === "number" \? check\.compressionArtifact\.blockScore : null/, "certification should retain compression artifact score");
  assert.match(appSource, /sharpenHaloScore: typeof check\.sharpenHalo\?\.haloScore === "number" \? check\.sharpenHalo\.haloScore : null/, "certification should retain sharpen halo score");
  assert.match(appSource, /fullSizeEdgeScore: typeof check\.fullSizeEdge\?\.score === "number" \? check\.fullSizeEdge\.score : null/, "certification should retain final-size edge score");
